...
const MovieSwiper = ({ data }) => {
  ...
  // ✅ If `data` prop is given, use it. Otherwise fallback to context
  const combinedData = data || [...(movieData || []), ...(swiperData || [])];

  // ✅ Filter out empty/invalid entries and ensure we have valid data
  const validData = combinedData.filter(movie => 
    movie && 
    (movie.id || movie.title) && 
    movie.title !== undefined
  );

  const totalSlides = Math.max(0, Math.ceil(validData.length / slidesPerView) - 1);
  ...
  return (
    <div className="py-5 pl-[0.75rem] xl:pl-0 group relative">
      <div
          ...
          >
            {id ? (
              Object.entries(videoMap).map(([movieId, videoSrc], index) => {
                const movieData = validData.find(item => item.id === movieId);
                
                return (
                  <div 
                    key={index} 
                    className={`rounded-lg flex flex-col gap-2 ${movieId === id ? 'bg-white/14 pb-5' : ''}`} 
                    style={{ flex: `0 0 ${210 / slidesPerView}%` }}
                  >
                    <Link to={`/video/${movieId}`} className='h-full relative'>
                      <video
                        ...
                        muted
                        playsInline
                        className="w-full rounded-[10px] object-cover cursor-pointer border-1 border-white/20"
                      />
                      <span className='absolute bottom-2 left-2 flex items-end justify-between w-full text-white'>
                        <span className='flex items-center gap-2'>
                          ...
                          <h4>Trailer</h4>
                          <p>{movieData?.trailerDuration || ''}</p>
                        </span>
                      </span>
                    </Link>
                    <Link to={`/video/${movieId}`} className="text-[1rem] text-white hover:underline px-2">
                      {movieData?.title || 'Unknown Title'}
                    </Link>
                    <Link to={`/video/${movieId}`} className="text-[1rem] text-white/60 px-2">
                      Official Trailer
                    </Link>
                    <span className="flex items-center text-sm gap-3 px-2">
                      <span className="flex text-sm items-center gap-1 text-white">
                        ...
                        {formatNumber(movieData?.likeCount)}
                      </span>
              ...
              validData.map((movie) => (
                ...
                >
                  <Movie movie={movie} onOpenRate={handleOpenRate} />
                </div>
              ))
            )}
          </div>
      ...
      {/* Render RatePopUp at the MovieSwiper level */}
      {selectedMovie && (
        <RatePopUp 
          movie={selectedMovie} 
